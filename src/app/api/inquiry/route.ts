...
export async function POST(req: NextRequest) {
  try {
    ...
    const companyLine = company ? `<p style="margin: 0; color: #6b7280; font-size: 14px;">Company</p><p style="margin: 4px 0 0 0; color: #111827; font-size: 16px; font-weight: 600;">${company}</p>` : ''

    // 1. Notify TargetCode team
    await resend.emails.send({
      from: 'TargetCode <onboarding@resend.dev>',
      to: ['ilayda@targetcode.io', 'ilaydaydemr@gmail.com'],
      subject: `New GTM System Inquiry: ${name}`,
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
          ...
          <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 12px; padding: 20px; margin-bottom: 20px;">
            ...
            <p style="margin: 4px 0 ${company ? '16px' : '0'} 0; color: #111827; font-size: 16px; font-weight: 600;">${email}</p>
            ${companyLine}
          </div>
